...
def _build_parser() -> argparse.ArgumentParser:
    ...
    bc.add_argument(
        ...
        default=100,
        metavar="N",
        help="teacher collection episodes (default 100)",
    )
    bc.add_argument(
        "--epochs",
        type=int,
    ...
    rl.add_argument(
        ...
        type=Path,
        default=DEFAULT_BC_CKPT,
        help=f"input npz (default {DEFAULT_BC_CKPT.name})",
    )
    rl.add_argument(
        "--episodes",
        type=int,
        default=400,
        metavar="N",
        help="RL episodes (default 400)",
    )
    rl.add_argument(
        "--save",
        type=Path,
        default=DEFAULT_RL_CKPT,
        help=f"output path (default {DEFAULT_RL_CKPT.name})",
    )
    tune_rl = rl.add_argument_group("details (normally left alone)")
...
def _legacy_argv(argv: list[str]) -> list[str]:
    """Map the old --bc-episodes / --load + --episodes to subcommands."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.mode == "rl":
        if not args.load.is_file():
            raise SystemExit(f"no checkpoint: {args.load}")
