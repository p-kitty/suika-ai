...
The default is offline BC with bootstrap (`choose_x`) as the teacher:
  ...
  3) save the weights with the best real-game score
     (cumulative eval is not used, since move-count bias makes 'dying fast' win)
...
def main() -> None:
    ...
    def _is_better(score: float, steps: float, match: float) -> bool:
        # Real-game score first. On ties, moves survived, and on ties again, match.
