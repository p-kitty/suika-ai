...
Only merging, danger avoidance, burying, rolling accidents and valley growing are pinned.
"""
...
def test_grows_apple_in_pear_valley_when_held_and_next_are_one_smaller() -> None:
    # Grow in a valley only when held/next are both fruits one smaller than the walls.
...
def test_blocked_column_misses_but_shoulder_or_open_same_type_merges() -> None:
    # A column blocked directly above by a different type does not merge on a direct hit. A same type on the shoulder or open side can merge.
...
def test_merges_sandwiched_same_type_despite_foreign_slope() -> None:
    ...
    fruits = (left, right, grape)
    # It merges even when shifted by about the aim error of real play.
