...
def test_shallow_shoulder_of_high_same_type_does_not_merge() -> None:
    # A shallow shoulder of a big same type above is not a merge (not reached even if stopped by the wall).
    apple_r = fruit_radius(5)
...
def test_does_not_prefer_unreachable_high_same_type_column() -> None:
    # Prefer a same type on the reachable floor side over one blocked directly above with only a shallow shoulder.
