"""Unit tests for falling, rolling, collision pushes and merging. Move selection of the policy is not looked at."""
...
def test_same_type_floor_contact_merges() -> None:
    # When same types touch on the floor, they merge without being pushed apart.
    cherry_r = fruit_radius(0)
    a = Fruit(
        type=0,
        x=200,
...
def test_large_fruit_rolls_off_instead_of_wedging_on_wall() -> None:
    # A big fruit's center stops at the wall, but it pushes the support away and falls to the floor.
    pear_r = fruit_radius(6)
...
def test_foreign_floor_hit_slides_toward_wall() -> None:
    # When it makes floor contact with a different type from a slope, the pushed fruit slides near the wall.
    pear_r = fruit_radius(6)
...
def test_fruit_above_deleted_watermelon_falls() -> None:
    # Fruits whose support vanished in a watermelon merge do not stay in the air but fall to the floor.
...
def test_hit_from_left_pushes_existing_right() -> None:
    # An existing fruit hit on the left shoulder moves clearly to the right.
    orange_r = fruit_radius(4)
    orange = Fruit(
        type=4,
        x=220,
...
def test_merge_from_right_pushes_neighbor_right() -> None:
    # Merging from the right moves the right neighbor clearly to the right too.
    cherry_r = fruit_radius(0)
...
def test_foreign_center_drop_rolls_off() -> None:
    # Directly above a different type is not stable and rolls to the floor on one side.
...
def test_shallow_shoulder_of_same_type_still_merges() -> None:
    # Same types merge on touch, even on the shoulder.
    apple_r = fruit_radius(5)
    melon_r = fruit_radius(9)
    apple = Fruit(type=5, x=200, y=NORMALIZED_HEIGHT - apple_r, radius=apple_r, confidence=90)
    melon = Fruit(
...
def test_blocked_column_misses_but_open_same_type_merges() -> None:
    # A column blocked directly above by a different type does not merge on a direct hit. A same type on the open side can merge.
...
def test_merges_sandwiched_same_type_despite_foreign_slope() -> None:
    # A same type wedged in a valley of big fruits merges without escaping down a different type's slope even if the aim is slightly off.
    pear_r = fruit_radius(6)
    grape_r = fruit_radius(2)
