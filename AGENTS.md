...
1. `SimEnv(seed=...)` で再生し、手ごとに盤・逆転率・閉じ込め数・頭頂を記録する
2. **崩壊が目に見えた手は結果であって原因でないことが多い。**
   「最後に盤が綺麗だった手」まで遡ってから前へ読む
3. 疑わしい手の**全候補**を eval と結果（閉じ込め・逆転）で並べる。原因の手は
   たいてい「同点プラトーの中で悪い方を引いた」形をしている。
   上位が同点で並ぶなら、その差を作れる項が eval に無いということ
4. 直したい手が直るかは、**その 1 局面で重みを振って**確かめる。決定的で数秒
5. 広く見るときは複数トレースの局面集合で一致率と構造指標を測る。
