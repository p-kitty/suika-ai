...
This file is loaded automatically every session. **[NOTES.md](NOTES.md) is not**, and that asymmetry is where
work gets repeated: a verdict written there is invisible until someone opens it. So before proposing any change
to the policy, to a weight, or to the search, **open
[Do not redo these](NOTES.md#do-not-redo-these) and check the question is not already closed** — it is one
table at the top of that file. Doing this costs one read; skipping it has cost whole nights of A/B runs.

**Two more things to check before touching anything.** If you are on `master`, cut a branch
...
- **CLAUDE.md (this file)** … how to work. Commands, code conventions, how changes proceed,
...
|---|---|
| How to run tests / lint / scripts, default arguments | CLAUDE.md |
| Why a default has that value | NOTES.md (link from CLAUDE.md without the numbers) |
...
| Procedures such as "plug A/B changes into `_apply_variant`" | CLAUDE.md (the resulting numbers go to NOTES.md) |
...
| A module / script was added or renamed | README.md Layout (CLAUDE.md if it needs a convention) |

If you feel like writing details here, do not; link to a NOTES.md section instead.
...
**This file is what Claude Code reads automatically, so it is the only one guaranteed to be in context.**
Keep it to how to work; measurements and domain knowledge go to NOTES.md.
