...
def main() -> None:
    args = _parse_args()
    env = Env()
    # Parallelize choose_x candidate evaluation (simulate_drop) over processes. Process startup cost
    # is large, so it is created once at program startup and reused, not per move.
