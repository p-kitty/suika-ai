...
def _merge_pair(
    ...
    side_min = 0.08
    # 14.0 から半減 (2026-08-18)。ひっぱはほぼ travel 項だけで決まる (speed 項は
    # 合計の 1〜8%) ので、そっと触れた合体でも全速の衝突と同じキックが入る。
    # 14.0 では型 6 のギリギリ側面で vx 548 px/s (盤幅 400) まで出て、周りが
    # 空いている合体だと新実が盤を横断していた。
    travel_gain = 7.0
    speed_gain = 0.06
