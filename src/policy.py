...
def _ladder_anchor(
    ...
    for fruit in fruits:
        if fruit.type != max_t or not pen.is_wall_anchored(fruit, sign):
            continue
        if best is None or pen.wall_gap(fruit, sign) < pen.wall_gap(best, sign):
            best = fruit
    return best
...
def _ladder_window(anchor: Fruit, sign: int) -> tuple[float, float]:
    ...
    inner = anchor.radius + fruit_radius(anchor.type - 1) * 2.0 + pen.MERGE_SLACK
