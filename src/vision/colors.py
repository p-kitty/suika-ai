...
# watermelon の半径 / 盤面幅。壁の内側基準に直した分だけ旧基準の 0.24 を
# 伸ばした値 (0.2874) は、メロン+パインの間にデコポン/グレープを落とす
# 際どいくさびケースで検証すると大きすぎた (デコポンだけでなくグレープも
# 床に届かなくなる)。実機ではグレープは抜けてデコポンは引っかかるので、
# その閾値帯 (実測 0.2770〜0.2810) に収まる 0.280 に取り直してある。
