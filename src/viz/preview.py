...
# The 'this is what happens if dropped' mini board at the top right.
PRED_SCALE = 0.65
PRED_MARGIN = 12
PRED_TOP = 64  # leave room below the AUTO/LIVE badge

# simulate_drop once per move is already heavy. render_preview is called every frame (at least
# CAPTURE_FPS often), so reuse the result unless the board or aim changes.
...
def _draw_prediction_panel(
    ...
    """Show at the top right a mini view of 'the fruits on the board now' and 'what happens if dropped there'."""
    ...
    # Like mode_badge, alpha blending a full-screen copy is too heavy to call every frame,
    # so it is not used. Filled rectangles are drawn directly.
