...
- 異種中央狙い (`FOREIGN_AIM`): 真下の実が異種かつ中心帯 (`FOREIGN_AIM_CENTER_FRAC`) なら減点。真下が同種なら OK。`merges` では切らない（異種真上から転がって合体する抜け道を塞ぐ）。同種過多 (`EXCESS_SAME` = 超過1個あたり20)。谷・肩への異種積みは禁じない
- 大きい実の谷育成は、谷に同種があるときか held/next が両方とも壁よりひとつ小さいときに限る。それ以外の隙間埋めは通常減点 (`GAP_JUNK` は廃止のまま)
- レイアウト: 最大実は左右どちらでも端寄せを強く。大きい実どうしは近接。最大より端の小実が L 中心より下 (角ポケット) に来るのは強く減点 (`_big_layout_penalty`)
